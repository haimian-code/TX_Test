...
Godot 4.x 编写的轻量肉鸽流派强弱模拟验证器。项目用于加载一份策划配置，替换不同策略 API 实现，批量运行同一关卡，并输出胜率、耗时、伤害曲线和 JSON/CSV 结果。
...
该脚本覆盖配置加载、坏配置报错、策略输出、固定种子确定性、tick/turn 战斗结果、腐蚀流 DOT 覆盖、随机词缀记录、回合制攻击间隔和 JSON/CSV 导出。
...
- 自动测试脚本和数据审计脚本
- 简单可视化对比面板：点击“对比流派”后用文本条形图对比胜率、平均耗时、平均 HP、平均伤害
- 随机装备词缀：每局按随机种子给已选道具 roll 词缀，并在单局/批量结果中记录
...
审计中发现并修复过一个规则问题：早期回合制模式里怪物每回合都会攻击，没有尊重配置中的 `attack_interval`，导致 turn 模式下腐蚀流被系统性压低到 0% 胜率。现在回合制会按时间步进结算冷却、能量、DOT 和怪物攻击间隔。后续又调高了唯一关卡中怪物的生命和攻击，让胜率不再全部饱和为 100%，从而更适合验证流派强弱。
...
- 暴击爆发流平均耗时更短；
- 腐蚀持续流胜率更稳定，尤其在精英怪长战中容错更高。

如果批量模拟发现暴击流同时拥有更高胜率和更短耗时，说明当前怪物防御或持续伤害倍率不足，腐蚀流的启动收益没有覆盖前期损失。后续可以通过提高精英怪 HP、降低普通怪数量、提高腐蚀瓶倍率或让持续伤害绕过防御来继续验证。
